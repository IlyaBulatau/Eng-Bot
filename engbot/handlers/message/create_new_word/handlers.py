...
HANDLER_CREATE_NEW_WORD = ConversationHandler(
    ...
    states={
        StateEnum.eng_word.value: [
            MessageHandler(filters=filters.TEXT & ~filters.COMMAND & filters.Regex(r"^[a-zA-z\s]+$"), callback=receive_eng_word),
            MessageHandler(filters=filters.ALL & ~filters.COMMAND, callback=incorrectly_eng_word),
            ],
        StateEnum.translate.value: [
            MessageHandler(filters=filters.TEXT & ~filters.COMMAND & filters.Regex(r"^[А-Яа-я\s]+$"), callback=receive_translate),
            MessageHandler(filters=filters.ALL & ~filters.COMMAND, callback=incorrectly_eng_word),
            ]
    },
